...
async def cmd_confirm_payment(message: Message, bot: Bot, db: Database, config: Config):
    ...
    # Получаем ID платежа из команды в формате /confirm_payment_ID
    command_text = message.text
    if "_" not in command_text:
        await message.answer("Неверный формат команды. Используйте: /confirm_payment_ID_платежа")
        return

    try:
        # Извлекаем ID платежа после последнего символа "_"
        payment_id = int(command_text.split("_")[-1])
    except ValueError:
